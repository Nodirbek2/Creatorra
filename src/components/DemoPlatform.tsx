...
export default function DemoPlatform({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#F8FAFC] text-slate-800 overflow-y-auto font-sans flex flex-col antialiased">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full flex flex-col justify-start">
        
        {/* Dynamic Interactive Browser Mockup Frame representing creatorra.uz/demo */}
        <div className="bg-[#FAFBFD] border border-slate-200 rounded-3xl overflow-hidden shadow-xs flex flex-col flex-1 pb-8">
          
          {/* Browser Chrome Header displaying creatorra.uz/demo */}
          <div className="bg-white border-b border-slate-205 px-4 py-3 flex items-center justify-between gap-4 select-none">
            ...
            {/* Navigation back and forward buttons + refresh */}
            <div className="hidden sm:flex items-center space-x-1.5 text-slate-400 shrink-0">
              <ArrowLeft className="w-3.5 h-3.5 cursor-not-allowed opacity-40" />
              <ArrowLeft className="w-3.5 h-3.5 rotate-180 cursor-not-allowed opacity-40 shrink-0" />
              <div 
                onClick={() => {
                  setIsCourseUnlocked(localStorage.getItem('creatorra_demo_unlocked') === 'true');
                }}
            ...
            {/* Simulated Address Bar showing creatorra.uz/demo */}
            <div className="flex-1 max-w-xl mx-auto flex items-center justify-center bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-lg px-3 py-1.5 text-xs text-slate-600 font-medium transition-all shadow-inner">
              <Lock className="w-3.5 h-3.5 text-emerald-600 mr-1.5 shrink-0 fill-emerald-600/10" />
              <span className="text-slate-400 select-none font-normal shrink-0">https://</span>
              <span className="text-slate-800 font-bold tracking-tight">creatorra.uz</span>
              <span className="text-slate-600 font-medium truncate">
                /demo{activeTab === 'student' ? '' : activeTab === 'creator' ? '/dashboard' : '/auth'}
              </span>
              <span className="ml-2 px-1 py-0.5 bg-emerald-50 border border-emerald-100 rounded text-[8px] text-emerald-705 tracking-wider flex items-center justify-center leading-none font-black uppercase text-center scale-90 shrink-0">
                SSL
              </span>
            </div>

            {/* Perfect Alignment balancing elements */}
            <div className="hidden sm:block w-[72px] shrink-0"></div>
          </div>

          {/* Browser Container Workspace Padding */}
          <div className="p-3 sm:p-5 lg:p-7 flex-1 flex flex-col justify-start">
